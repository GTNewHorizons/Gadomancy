...
public class TileOverrideEldritchLock extends TileEldritchLock {
    ...
    public void updateEntity() {
        if (this.worldObj.isRemote || super.count == -1 || super.count + 1 <= 100) {
            super.updateEntity();
            return;
        }
        ...
        CellLoc lockChunk = new CellLoc(this.xCoord >> 4, this.zCoord >> 4);
        for (TCMazeSession session : TCMazeHandler.getSessions().values()) {
            if (session.chunksAffected == null || !session.chunksAffected.containsKey(lockChunk)) {
                continue;
            }
